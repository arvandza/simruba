...
@section('title', 'SIMRUBA | Pengajuan Peminjaman Ruangan')

@section('content')

    @section('current_menu')Menu Pengajuan @endsection
    @section('current_page')Pengajuan Peminjaman Ruangan @endsection
            ...
            <div class="card">
                <div class="card-body">
                    <div class="d-flex flex-wrap align-items-center">
                        ...
                        <h4 class="card-title mb-0">Peminjaman Ruangan</h4>
                    </div>
                    <p class="text-muted mb-3 mt-3">Silahkan isi form peminjaman</p>
                    <form action="{{ route('roomlist.request') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('POST')
                        <input type="hidden" name="user_id" value="1">
                        <input type="hidden" name="room_id" value="{{ $roomLoan->id }}">
                        <div class="mb-3">
                            <label for="image" class="form-label">Gambar Ruangan<span
                                    class="text-danger">*</span></label>
                            <input type="file" id="dropify" name="image" accept="image/*"
                                data-default-file="{{ asset('storage/' . $roomLoan->image) }}" disabled="disabled" />
                        </div>
                        <div class="mb-3">
                            <label for="name" class="form-label">Nama Ruangan<span class="text-danger">*</span></label>
                            <input id="name" class="form-control" name="name" type="text"
                                value="{{ $roomLoan->name }}" readonly>
                        </div>
                        <div class="row">
                            <div class="col-sm-6">
                                <div class="mb-3">
